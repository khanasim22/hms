...
if (!function_exists('generateCustomSerialNumber')) {
    function generateCustomSerialNumber($modelClass, $fieldName)
    {
        ...
        $customPrefixRaw = trim($settings['custom_serial_prefix'] ?? '');

        preg_match('/\d+$/', $customPrefixRaw, $matches);
        $customStartNumber = $matches[0] ?? '1';

        $customPrefix = preg_replace('/\d+$/', '', $customPrefixRaw);

        if ($customSrNoEnable && !empty($customPrefix)) {

            $latestRecord = $modelClass::where($fieldName, 'LIKE', $customPrefix . '%')
                ->orderByRaw("CAST(REGEXP_SUBSTR($fieldName, '[0-9]+$') AS UNSIGNED) DESC")
                ->first();

            if ($latestRecord) {
                $latestNumber = $latestRecord->$fieldName;

                if (preg_match('/(\d+)$/', $latestNumber, $matches)) {
                    $numberPart = $matches[1];
                    $numberLength = strlen($numberPart);

                    $incrementedNumber = str_pad(
                        ((int)$numberPart + 1),
                        $numberLength,
                        '0',
                        STR_PAD_LEFT
                    ...
                    $newSerial = preg_replace('/(\d+)$/', $incrementedNumber, $latestNumber);
                } else {
            ...
            if ($modelClass === MedicineBill::class) {
                ...
                } while ($modelClass::where($fieldName, '=', 'BIL' . $code)->exists());
