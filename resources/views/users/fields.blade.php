...
<div class="row">
    ...
    @if (!$isEdit)
        ...
        <div class="col-lg-6 mb-5" id="doctorDepartmentField" style="{{ old('department_id') == 2 ? '' : 'display:none;' }}">
            {{ Form::label('doctor_department', __('messages.doctor_departments') . ':', ['class' => 'form-label']) }}
            <span class="text-danger">*</span>
            <select name="doctor_department" class="form-select"  data-control="select2" required>
                <option value="">{{ __('messages.web_appointment.select_department') }}</option>
                @foreach($doctorDepartments as $id => $title)
                <option value="{{ $id }}" {{ old('doctor_department') == $id ? 'selected' : '' }}>
                    {{ $title }}
                </option>
                @endforeach
            </select>
